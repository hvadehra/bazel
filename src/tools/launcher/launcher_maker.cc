...
int main(int argc, char** argv) {
  ...
  std::wstring wlauncher_path = windows_path(launcher_path);
  std::ifstream src(wlauncher_path, std::ios::binary);
  if (!src.good()) {
    fprintf(stderr, "Failed to open %ls: %s\n", wlauncher_path.c_str(), strerror(errno));
    return 1;
  }
  std::wstring woutput_path = windows_path(output_path);
  std::ofstream dst(woutput_path, std::ios::binary);
  if (!dst.good()) {
    fprintf(stderr, "Failed to create %ls: %s\n", woutput_path.c_str(), strerror(errno));
    return 1;
  }
  dst << src.rdbuf();

  std::wstring winfo_params = windows_path(info_params);
